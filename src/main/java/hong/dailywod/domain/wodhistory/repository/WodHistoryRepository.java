...
public interface WodHistoryRepository
    ...
    @Query(
            "SELECT wh FROM WodHistory wh JOIN wh.wod w WHERE w.wodDate BETWEEN :startDate AND :endDate AND wh.user.id = :userId")
    List<WodHistory> findAllByWodDateBetweenAndUserId(
